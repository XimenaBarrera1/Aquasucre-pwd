...
async function cargarOrdenes() {
  try {
    ...
    os.forEach(o => {
      $('ordenes').insertAdjacentHTML(
        'beforeend',
        `<div class="card" onclick='abrir(${JSON.stringify(o)})'><b>OT #${o.id_ot}</b><br>${o.tipo_servicio}<br><span class="estado">${o.estado}</span> · ${o.prioridad}</div>`
      );
    });
  } catch (e) {
    mensaje(e.message);
...
async function iniciar() {
  try {
    await api(`/api/ordenes/${actual.id_ot}/iniciar`, {
      method: 'POST',
